...
public class MainServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		if (req.getParameter("operation").equals("insertSubscription")) {
		...
		} else if (req.getParameter("operation").equals("cronjob")) {
			Cron.setStarted(true);
			new Thread(new Cron()).start();
			message = "Cronjob has been started.";
			...
			Location location = MirrorClient.getMirror(credential).locations()
					.get("latest").execute();
			// timelineItem.setNotification(new
			// NotificationConfig().setLevel("DEFAULT"));
			// timelineItem.setLocation(new
			// Location().setAddress("Haagdijk 54, Breda"));
